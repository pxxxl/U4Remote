config_path = "/home/ethan/Project/Python/I3DV/i3DV1.0/configs/Dance_Dunhuang_Pair_1080/Entropy_Skipping/Dance_Dunhuang_Pair_1080_0.0001_2.0_0.3.json"

# will use entropy skipping to fine ture the model
...
def training_I_frame_finetune(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None, init=True):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    ...
    gaussians.set_steps(args_param.step_flag1, args_param.step_flag2)
    gaussians.set_entropy_skipping(args_param.entropy_skipping_finetune_ratio)
    gaussians.set_learned_anchor_digits(args_param.enable_learned_anchor_digits)
    gaussians.set_finetune_params(args_param.entropy_skipping_finetune_iterations, args_param.entropy_skipping_finetune_ratio)

    if init:
        scene = Scene(dataset, gaussians, ply_path=ply_path, load_iteration=saving_iterations[0])
        gaussians.update_anchor_bound()
    else:
        scene = Scene_of_frame(dataset, gaussians, ply_path=ply_path, init_points=args_param.init_points)
        gaussians.update_anchor_bound()
    ...
    viewpoint_stack = None
    ema_loss_for_log = 0.0
    progress_bar = tqdm(range(first_iter, args_param.entropy_skipping_finetune_iterations), desc="Finetune progress")
    first_iter += 1
    torch.cuda.synchronize(); t_start = time.time()
    log_time_sub = 0
    for iteration in range(first_iter, args_param.entropy_skipping_finetune_iterations + 1):
        # network gui not available in scaffold-gs yet
        if network_gui.conn == None:
            network_gui.try_connect()
        while network_gui.conn != None:
...
if __name__ == "__main__":
    ...
    for frame in range(frame_start, frame_end+1):
        ...
        if frame % GOF == 0: 
            ...
            args.entropy_skipping_finetune = None
            args.entropy_skipping_finetune_iterations = None
            args.entropy_skipping_finetune_ratio = None
            ...
            training_I_frame_finetune(args, lp.extract(args), op.extract(args), pp.extract(args), dataset,  args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, wandb, logger, init=(frame==0))
